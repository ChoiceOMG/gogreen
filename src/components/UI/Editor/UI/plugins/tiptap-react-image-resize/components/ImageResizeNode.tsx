...
import { ImageSizeExtensionOptions } from "..";
...
export const AutoSizeImage = (props: any) => {
  ...
  const getValidWidth = useCallback(
    (_width: string | number) => {
      ...
      if (width < covertPxToNum(options.minWidth))
        return covertPxToNum(options.minWidth);
      if (width > covertPxToNum(options.maxWidth))
        return covertPxToNum(options.maxWidth);
        ...
        <div
          ...
          style={{
            outline: props.selected
              ? `2px solid ${options.activeBorderColor}`
              : "none",
            width: "max-content",
          }}
        >
            ...
            >
              {options.levels.map((level, index) => (
                <div
                  key={level}
                  title={`${level} px`}
                  className="image-resize-plugin-icon"
                  onClick={() => setImageWidth(level)}
                >
                  <IconResize size={iconSizes[index]} />
                </div>
              ))}
              <div
                title="max"
                className="image-resize-plugin-icon"
                onClick={() => setImageWidth(options.maxWidth)}
              >
                <IconResize size={24} />
              </div>
              <div
